...
ORDER BY trade_date'''
        ...
        self.select_valid_record_by_date_where_clause = '''
WHERE
    open IS NOT NULL
        AND trade_date = %s
ORDER BY trade_date'''
    ...
    def select_valid_record_by_date(self, date):
        sql = self.select_ts_code_sql + self.select_valid_record_by_date_where_clause
        return self.select(sql, [date.strftime("%Y%m%d")])
