...
import libs.pandas
import libs.mysql
from libs.common import mysql, pandas
...
def stat_all_lite(date):
    ...
    try:
        data = pd.read_sql(sql=sql_1, con=libs.mysql.engine(), params=[date.strftime("%Y%m%d")])
        data = data.drop_duplicates(subset="code", keep="last")
    except Exception as e:
        print('error :', e)

    try:
        libs.mysql.insert_db(data, "guess_indicators_lite_daily", False, "`date`,`code`")
    except Exception as e:
        print("error :", e)
