...
class TestTensorflowSlice(unittest.TestCase):
    ...
    def test_temsorflow_comm_slice(self):
        date = datetime.str_to_date('20180331')
        ...
        file_system.write(data, segment=datetime.date_to_str(date), file_name=file_name)

        result = file_system.read(segment=datetime.date_to_str(date), file_name=file_name)
