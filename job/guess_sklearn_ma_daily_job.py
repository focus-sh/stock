...
def stat_all_batch(tmp_datetime):
    ...
    count = lib.mysql.select_count(sql_count, params=[datetime_int, '300%', '%st%'])
    print("count :", count)
    batch_size = 100
    ...
    for i in range(0, end, batch_size):
        ...
        data = pd.read_sql(sql=sql_1, con=lib.mysql.engine(), params=[datetime_int, '300%', '%st%', i, batch_size])
        ...
        try:
            lib.mysql.insert_db(data_new, table_name, False, "`date`,`code`")
            print("insert_db")
        except Exception as e:
            print("error :", e)
        # 重命名
